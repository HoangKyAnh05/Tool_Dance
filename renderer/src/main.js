...
import { DanceInstructor } from './engine/danceInstructor';
class App {
    poseTracker;
    audioManager;
    videoExporter;
    ...
    initProjectsSystem() {
        ...
        const titleInput = document.getElementById('save-project-title-input');
        const notesInput = document.getElementById('save-project-notes-input');
    ...
    restoreLastSession() {
        try {
            ...
            if (last) {
                ...
                if (data.speed)
                    this.setPlaybackSpeed(data.speed);
            }
        }
        catch (e) {
            // ignore
    ...
    startLoop() {
        const loop = () => {
            ...
            if (this.instCurrentTimeEl.textContent !== timeStr)
                this.instCurrentTimeEl.textContent = timeStr;
            const beatStr = `NHỊP ${inst.beatIndex}/8`;
            if (this.currentBeatBadgeEl.textContent !== beatStr)
                this.currentBeatBadgeEl.textContent = beatStr;
            const titleSpan = this.instMoveTitleEl.querySelector('span');
            if (titleSpan && titleSpan.textContent !== inst.moveName)
                titleSpan.textContent = inst.moveName;
            if (this.instLeftArmEl.textContent !== inst.leftArmDesc)
                this.instLeftArmEl.textContent = inst.leftArmDesc;
            if (this.instRightArmEl.textContent !== inst.rightArmDesc)
                this.instRightArmEl.textContent = inst.rightArmDesc;
            if (this.instTorsoEl.textContent !== inst.torsoDesc)
                this.instTorsoEl.textContent = inst.torsoDesc;
            if (this.instLeftLegEl.textContent !== inst.leftLegDesc)
                this.instLeftLegEl.textContent = inst.leftLegDesc;
            if (this.instRightLegEl.textContent !== inst.rightLegDesc)
                this.instRightLegEl.textContent = inst.rightLegDesc;
            const angleStr = `Tay: L ${inst.leftElbowAngle}° | R ${inst.rightElbowAngle}°  •  Gối: L ${inst.leftKneeAngle}° | R ${inst.rightKneeAngle}°`;
            if (this.instJointAnglesEl.textContent !== angleStr)
                this.instJointAnglesEl.textContent = angleStr;
