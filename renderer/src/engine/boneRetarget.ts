...
export interface RigPoseTarget {
  ...
  leftLeg: {
    upperLeg: THREE.Euler;
    lowerLeg: THREE.Euler;
  };
  rightLeg: {
    upperLeg: THREE.Euler;
    lowerLeg: THREE.Euler;
...
export class BoneRetargeter {
   ...
   */
  public retarget(landmarks: Landmark[]): RigPoseTarget {
    const defaultPose: RigPoseTarget = {
    ...
    if (!landmarks || landmarks.length < 29) return defaultPose;
