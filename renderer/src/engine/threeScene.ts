...
import { CharacterPreset, STAGE_THEMES, StageTheme } from '../presets';
...
export class ThreeScene {
  ...
  // Characters
  public dancers: CharacterRig[] = [];
  public mainDancer: CharacterRig | null = null;
  
  // Stage Elements
  private floorGrid: THREE.GridHelper | null = null;
  private floorMirror: THREE.Mesh | null = null;
  ...
  constructor(container: HTMLElement, defaultPreset: CharacterPreset) {
  ...
  public setMainDancer(preset: CharacterPreset) {
    ...
    const leftDancerPreset: CharacterPreset = {
      ...preset,
    ...
    const rightDancerPreset: CharacterPreset = {
      ...preset,
      scale: preset.scale * 0.92,
      primaryColor: '#f472b6',
      accentColor: '#fbbf24',
  ...
  public setStageTheme(theme: StageTheme) {
  ...
  public updateDancePose(pose: RigPoseTarget, currentTime: number, beatIntensity: number = 0) {
    ...
    for (let i = 1; i < this.dancers.length; i++) {
      const delayPose: RigPoseTarget = {
        ...pose,
        rootPosition: new THREE.Vector3(
          (i === 1 ? -1.6 : 1.6) + pose.rootPosition.x * 0.8,
          pose.rootPosition.y,
          -0.4
        ),
      };
    ...
    if (this.particleField) {
      const pos = this.particleField.geometry.attributes.position.array as Float32Array;
